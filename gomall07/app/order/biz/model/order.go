...
type Order struct {
	...
	Consignee    Consignee   `gorm:"embedded"` // 可以嵌入一个新的结构体
...
func ListOrder(ctx context.Context, db *gorm.DB, userId uint32) ([]*Order, error) {
	var orders []*Order
	err := db.WithContext(ctx).Where("user_id=?", userId).Preload("OrderItems").Find(&orders).Error // Preload("CreditCards")关联表附加
	if err != nil {
		return nil, err
	}
	return orders, nil
